...
function ResourceView({ module, session, online, onDataChanged, onNavigate }) {
  ...
  const canCreate = online && !module.readOnly && permissionAllows(session, "create", module.resource) && (module.id !== "accounting" || canViewOfficial);
  ...
  useEffect(() => { load(""); }, [module.id, session?.tenant?.id, officialFilter]);
